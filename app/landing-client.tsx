...
export function LandingClient({ isLoggedIn = false }: { isLoggedIn?: boolean }) {
  ...
  const features = [
    { icon: "🤖", titleKey: "landing.feat1Title" as const, descKey: "landing.feat1Desc" as const, light: "bg-violet-50 border-violet-100", dark_: "bg-violet-500/8 border-violet-500/15", iLight: "bg-violet-100 text-violet-600", iDark: "bg-violet-500/15 text-violet-400" },
    { icon: "📄", titleKey: "landing.feat2Title" as const, descKey: "landing.feat2Desc" as const, light: "bg-blue-50 border-blue-100", dark_: "bg-blue-500/8 border-blue-500/15", iLight: "bg-blue-100 text-blue-600", iDark: "bg-blue-500/15 text-blue-400" },
    { icon: "🇹🇭", titleKey: "landing.feat3Title" as const, descKey: "landing.feat3Desc" as const, light: "bg-green-50 border-green-100", dark_: "bg-green-500/8 border-green-500/15", iLight: "bg-green-100 text-green-600", iDark: "bg-green-500/15 text-green-400" },
    { icon: "📥", titleKey: "landing.feat4Title" as const, descKey: "landing.feat4Desc" as const, light: "bg-orange-50 border-orange-100", dark_: "bg-orange-500/8 border-orange-500/15", iLight: "bg-orange-100 text-orange-600", iDark: "bg-orange-500/15 text-orange-400" },
    { icon: "🎨", titleKey: "landing.feat5Title" as const, descKey: "landing.feat5Desc" as const, light: "bg-pink-50 border-pink-100", dark_: "bg-pink-500/8 border-pink-500/15", iLight: "bg-pink-100 text-pink-600", iDark: "bg-pink-500/15 text-pink-400" },
    { icon: "🔗", titleKey: "landing.feat6Title" as const, descKey: "landing.feat6Desc" as const, light: "bg-indigo-50 border-indigo-100", dark_: "bg-indigo-500/8 border-indigo-500/15", iLight: "bg-indigo-100 text-indigo-600", iDark: "bg-indigo-500/15 text-indigo-400" },
  ]

  const steps = [
    { step: "01", icon: "✍️", titleKey: "landing.step1Title" as const, descKey: "landing.step1Desc" as const, lightCard: "from-orange-50 to-amber-50 border-orange-100", darkCard: "from-orange-500/8 to-amber-500/8 border-orange-500/15", lightBadge: "bg-orange-100 text-orange-600", darkBadge: "bg-orange-500/15 text-orange-400", delay: "d1" },
    { step: "02", icon: "✦", titleKey: "landing.step2Title" as const, descKey: "landing.step2Desc" as const, lightCard: "from-indigo-50 to-violet-50 border-indigo-100", darkCard: "from-indigo-500/8 to-violet-500/8 border-indigo-500/15", lightBadge: "bg-indigo-100 text-indigo-600", darkBadge: "bg-indigo-500/15 text-indigo-400", delay: "d2" },
    { step: "03", icon: "🌐", titleKey: "landing.step3Title" as const, descKey: "landing.step3Desc" as const, lightCard: "from-emerald-50 to-teal-50 border-emerald-100", darkCard: "from-emerald-500/8 to-teal-500/8 border-emerald-500/15", lightBadge: "bg-emerald-100 text-emerald-600", darkBadge: "bg-emerald-500/15 text-emerald-400", delay: "d3" },
  ]
        ...
        <section className="relative z-10 min-h-[92vh] flex items-center overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col items-center text-center px-5 sm:px-6 py-24 sm:py-36 max-w-5xl mx-auto w-full">
            ...
            <div className={`flex flex-wrap justify-center items-center gap-3 sm:gap-6 text-xs sm:text-sm ${mutedText}`}>
              ...
              <span className={`hidden sm:block ${dark ? "text-white/10" : "text-gray-200"}`}>|</span>
              <span className="flex items-center gap-1.5"><span className="text-green-500">✓</span> {t("landing.proof3")}</span>
            </div>
          </div>
        ...
        <footer className={`relative z-10 border-t py-7 sm:py-8 px-5 sm:px-8 transition-colors duration-500 ${dark ? "border-white/8" : "border-gray-100"}`}>
          <div className="max-w-5xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3 sm:gap-4 text-center sm:text-left">
            ...
            <div className={`flex gap-4 text-xs ${mutedText}`}>
              <Link href="/login" className={`transition-colors ${dark ? "hover:text-gray-200" : "hover:text-gray-700"}`}>{t("landing.login")}</Link>
              <Link href="/register" className={`transition-colors ${dark ? "hover:text-gray-200" : "hover:text-gray-700"}`}>{t("landing.cta")}</Link>
            </div>
          </div>
        </footer>
